...
/**
 * Endpoint público (fora de /api, sem sessão autenticada) usado para gravar testes
 * capturando eventos diretamente na aba do próprio usuário — via bookmarklet ou
 * snippet colado no console — em vez de abrir um Chrome controlado por Selenium
 * no servidor (que fica invisível em produção, pois o servidor não tem display).
 *
...
public class CapturaPublicaController {
    ...
    @GetMapping(value = "/script.js", produces = "application/javascript;charset=UTF-8")
    public ResponseEntity<String> script(@RequestParam String token, @RequestParam String eventoUrl) {
        if (!gravacaoService.tokenValido(token)) {
            return ResponseEntity.status(403)
                    .contentType(MediaType.valueOf("application/javascript;charset=UTF-8"))
                    .body("console.error('[Qorbit] Token de gravação inválido ou expirado.');");
        }
        String js = TEMPLATE
                .replace("__TOKEN__", token.replace("\"", ""))
                .replace("__EVENTO_URL__", eventoUrl.replace("\"", ""));
    ...
    private static final String TEMPLATE = """
        (function() {
          if (window.__qorbitGravando) { console.warn('[Qorbit] Já está gravando nesta aba.'); return; }
          window.__qorbitGravando = true;

          var TOKEN = "__TOKEN__";
          var EVENTO_URL = "__EVENTO_URL__";
          ...
          function enviar(ev) {
            try {
              fetch(EVENTO_URL, {
                ...
                body: JSON.stringify(Object.assign({ token: TOKEN }, ev))
              }).catch(function () {});
            } catch (e) {}
          }
          ...
          function mostrarBanner() {
            try {
              ...
              b.textContent = '⏺ Qorbit gravando — volte para a aba do Qorbit para parar';
              b.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:2147483647;' +
                'background:#dc2626;color:#fff;font:12px/1.4 sans-serif;text-align:center;' +
                'padding:6px 10px;box-shadow:0 1px 4px rgba(0,0,0,.3)';
