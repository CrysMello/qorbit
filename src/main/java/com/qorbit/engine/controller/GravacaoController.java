...
public class GravacaoController {
    ...
    public ResponseEntity<?> iniciar(@RequestBody Map<String, String> body) {
        ...
        try {
            ...
            // O serviço roda como root no servidor (Xvfb :99) — sem --no-sandbox o
            // Chrome recusa abrir ("Running as root without --no-sandbox is not supported").
            opts.addArguments("--no-sandbox", "--disable-dev-shm-usage");
            opts.addArguments("--window-size=1280,800", "--window-position=0,0");
            ...
            // .maximize() depende de um window manager (falha em Xvfb puro, sem WM);
            // o tamanho já é definido via --window-size acima.
