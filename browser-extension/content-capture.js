// Roda em toda página (<all_urls>). Só ativa a captura de fato quando existe uma
// sessão de gravação ativa no chrome.storage (setada pelo background.js quando o
// usuário clica "Iniciar gravação" no Qorbit). Sem sessão ativa, este script não
// faz nada além de checar o storage — não observa nem envia nada.

(function () {
  if (window.__qorbitGravando) return;
  ...
  var jaTentouBoot = false;
  function tentarBoot() {
    if (jaTentouBoot || window.__qorbitGravando) return;
    chrome.storage.local.get('qorbitSessao', function (r) {
      if (r && r.qorbitSessao) {
        jaTentouBoot = true;
  ...
  try {
    chrome.storage.onChanged.addListener(function (changes, area) {
      if (area === 'local' && changes.qorbitSessao && changes.qorbitSessao.newValue) tentarBoot();
    });
  } catch (e) {}
})();
